...
app.get('/', (req, res) => res.send('VIRU FM - 24/7 ULTIMATE STABLE MODE! 🛡️🔊'));
...
function startStreaming() {
    ...
    // ✅ සියලුම .mp3 ෆයිල් අහු කරගන්නවා (Case insensitive)
    ...
    // ✅ ප්ලේලිස්ට් එකේ Paths හරියටම හදනවා (Fix for spaces and special characters)
    ...
    console.log("Starting Lifetime Stream: Absolute Path + Re-encode Mode...");
    ...
    const ffmpeg = spawn('ffmpeg', [
        ...
        '-filter_complex', 
        // 🛡️ [2:a] කියන්නේ සින්දු ටික. ඒක re-sample කරලා ප්ලේ කරන නිසා corruption අඩු වෙනවා.
    ...
    ffmpeg.stderr.on('data', (data) => {
        ...
        if (msg.includes('Error') || msg.includes('corrupt')) {
...
app.listen(port, '0.0.0.0', () => {
    console.log(`Viru FM Server Active on Port ${port}`);
    if (STREAM_KEY) {
        startStreaming();
    } else {
        console.log("CRITICAL: STREAM_KEY is missing!");
    }
});
